...
bool SystemData::loadConfig()
{
    ...
    for (auto& configPath : configPaths) {
        ...
        if (doc.child("loadExclusive") || systemList.child("loadExclusive"))
            break;
    }
    ...
    for (auto& configPath : configPaths) {
        ...
        const pugi::xml_node& systemList {doc.child("systemList")};
        const pugi::xml_node& loadExclusive {
            doc.child("loadExclusive") ? doc.child("loadExclusive")
                                       : systemList.child("loadExclusive")};
        if (loadExclusive) {
        ...
        // Actually read the file.
        const pugi::xml_node& systemListParsed {doc.child("systemList")};
        ...
        for (pugi::xml_node system {systemListParsed.child("system")}; system;
             system = system.next_sibling("system")) {
...
{
    const auto getLocalizedSystemName = [](const std::string& systemName,
                                           const std::string& fallbackName) {
        if (systemName == "arcade")
            return std::string {"Аркадные автоматы"};
        if (systemName == "neogeo")
            return std::string {"SNK Neo Geo"};
        if (systemName == "nes")
            return std::string {"Nintendo Entertainment System (NES)"};
        if (systemName == "famicom")
            return std::string {"Nintendo Family Computer (Famicom)"};
        if (systemName == "fds")
            return std::string {"Nintendo Famicom Disk System"};
        if (systemName == "snes")
            return std::string {"Super Nintendo Entertainment System (SNES)"};
        if (systemName == "sfc")
            return std::string {"Super Famicom"};
        if (systemName == "gb")
            return std::string {"Nintendo Game Boy"};
        if (systemName == "gbc")
            return std::string {"Nintendo Game Boy Color"};
        if (systemName == "gba")
            return std::string {"Nintendo Game Boy Advance"};
        if (systemName == "mastersystem")
            return std::string {"Sega Master System"};
        if (systemName == "gamegear")
            return std::string {"Sega Game Gear"};
        if (systemName == "megadrive")
            return std::string {"Sega Mega Drive"};
        if (systemName == "genesis")
            return std::string {"Sega Genesis"};
        if (systemName == "segacd")
            return std::string {"Sega CD"};
        if (systemName == "megacd")
            return std::string {"Sega Mega-CD"};
        if (systemName == "pcengine")
            return std::string {"NEC PC Engine"};
        if (systemName == "tg16")
            return std::string {"NEC TurboGrafx-16"};
        if (systemName == "pcenginecd")
            return std::string {"NEC PC Engine CD"};
        if (systemName == "tg-cd")
            return std::string {"NEC TurboGrafx-CD"};
        if (systemName == "ngp")
            return std::string {"SNK Neo Geo Pocket"};
        if (systemName == "ngpc")
            return std::string {"SNK Neo Geo Pocket Color"};
        if (systemName == "wonderswan")
            return std::string {"Bandai WonderSwan"};
        if (systemName == "wonderswancolor")
            return std::string {"Bandai WonderSwan Color"};
        if (systemName == "psx")
            return std::string {"Sony PlayStation"};
        if (systemName == "dos")
            return std::string {"MS-DOS"};
        if (systemName == "amiga")
            return std::string {"Commodore Amiga"};
        if (systemName == "amigacd32")
            return std::string {"Commodore Amiga CD32"};
        if (systemName == "msx")
            return std::string {"MSX"};
        if (systemName == "msx2")
            return std::string {"MSX2"};
        if (systemName == "c64")
            return std::string {"Commodore 64"};
        return fallbackName;
    };
    ...
    for (auto& configPath : configPaths) {
        ...
        for (pugi::xml_node system {systemList.child("system")}; system;
            ...
            themeFolder = system.child("theme").text().as_string(name.c_str());
            const bool isCustomSystem {configPaths.size() != 1 && configPath == configPaths.back()};
            const std::string localizedFullname {getLocalizedSystemName(name, fullname)};
            ...
            systemInfoFile << "Системное имя:" << std::endl;
            if (isCustomSystem)
                systemInfoFile << name << " (пользовательская система)" << std::endl
                               << std::endl;
            else
                systemInfoFile << name << std::endl << std::endl;
            systemInfoFile << "Полное название системы:" << std::endl;
            systemInfoFile << localizedFullname << std::endl << std::endl;
            systemInfoFile << "Поддерживаемые расширения файлов:" << std::endl;
            systemInfoFile << extensions << std::endl << std::endl;
            systemInfoFile << "Команда запуска:" << std::endl;
            systemInfoFile << commands.front() << std::endl << std::endl;
            // Alternative emulator configuration entries.
            if (commands.size() > 1) {
                systemInfoFile << (commands.size() == 2 ? "Альтернативная команда запуска:" :
            ...
            systemInfoFile << "Платформ" << (multiplePlatforms ? "ы" : "а")
                           << " для скрапинга:" << std::endl;
            if (platform.empty())
                systemInfoFile << "Не задано" << std::endl << std::endl;
            else
                systemInfoFile << platform << std::endl << std::endl;
            systemInfoFile << "Папка темы:" << std::endl;
            ...
            if (isCustomSystem)
                systemsVector.emplace_back(std::make_pair(systemDir + " (пользовательская система)",
                                                          localizedFullname));
            else
                systemsVector.emplace_back(std::make_pair(systemDir, localizedFullname));
