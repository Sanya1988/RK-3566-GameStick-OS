    ...
    , mInvalidFolderLinkEntry {false}
{
    ...
    const struct {
        ...
        SystemData* system;
        bool automaticMode;
    } scraperParams {mGame, mSystem, false};
#endif
    ...
    std::string folderPath {
        Utils::String::replace(Utils::FileSystem::getParent(mGame->getPath()),
                               mSystem->getSystemEnvData()->mStartPath, "")};
    ...
    mSubtitle = std::make_shared<TextComponent>(
        folderPath + Utils::FileSystem::getFileName(mGame->getPath()) + " [" +
            Utils::String::toUpper(mSystem->getName()) + "]" +
            (mGame->getType() == FOLDER ? "  " + ViewController::FOLDER_CHAR : ""),
            ...
            case MD_ALT_EMULATOR: {
                ...
                if (originalValue != "" &&
                    ...
                    LOG(LogWarning)
                        ...
                        << "\" configured for game \"" << mGame->getName() << "\"";
...
void GuiMetaDataEd::save()
{
    ...
    mSystem->getIndex()->removeFromIndex(mGame);
    ...
    if (hideGameWhileHidden)
        GamelistFileParser::updateGamelist(mSystem);

    // Enter game in index.
    if (mGame->getType() == GAME)
        mSystem->getIndex()->addToIndex(mGame);

    // If it's a folder that has been updated, we need to manually sort the gamelist
    // as CollectionSystemsManager ignores folders.
    if (mGame->getType() == FOLDER)
        mSystem->sortSystem(false);
    ...
    if (hideGameWhileHidden) {
        ...
        if (mGame->getType() == FOLDER) {
            for (FileData* child : mGame->getChildrenRecursive()) {
                if (!child->getHidden())
        ...
        CollectionSystemsManager::getInstance()->refreshCollectionSystems(mGame);
    }

    // If game counting was re-enabled for the game, then reactivate it in any custom collections
    // where it may exist.
    if (setGameAsCounted)
        CollectionSystemsManager::getInstance()->reactivateCustomCollectionEntry(mGame);

    mSystem->onMetaDataSavePoint();
...
void GuiMetaDataEd::close()
{
    ...
    closeFunc = [this] {
        if (mMediaFilesUpdated || mSavedMediaAndAborted) {
            ...
            TextureResource::manualUnload(mGame->getImagePath(), false);
            TextureResource::manualUnload(mGame->getMarqueePath(), false);
