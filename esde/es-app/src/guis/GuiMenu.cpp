...
void GuiMenu::openUIOptions()
{
    ...
    for (auto& localeEntry : Utils::Localization::sSupportedLocales) {
        std::string localeName {localeEntry.first};
        if (localeEntry.second != "")
            localeName.append("_").append(localeEntry.second);

        applicationLanguage->add(
            Utils::String::toUpper(_(ThemeData::getLanguageLabel(localeName).c_str())), localeName,
            selectedApplicationLanguage == localeName);
    }
    // If there are no objects returned, then there must be a manually modified entry in the
...
// TODO: Implement system volume support for macOS and Android.
    ...
    auto soundVolumeBackgroundMusic = std::make_shared<SliderComponent>(0.0f, 100.0f, 1.0f, "%");
    soundVolumeBackgroundMusic->setValue(
        static_cast<float>(Settings::getInstance()->getInt("SoundVolumeBackgroundMusic")));
    s->addWithLabel(_("BACKGROUND MUSIC VOLUME"), soundVolumeBackgroundMusic);
    s->addSaveFunc([soundVolumeBackgroundMusic, s] {
        if (soundVolumeBackgroundMusic->getValue() !=
            static_cast<float>(Settings::getInstance()->getInt("SoundVolumeBackgroundMusic"))) {
            Settings::getInstance()->setInt(
                "SoundVolumeBackgroundMusic",
                static_cast<int>(soundVolumeBackgroundMusic->getValue()));
            s->setNeedsSaving();
        }
    });
    ...
    if (UIModeController::getInstance()->isUIModeFull()) {
        ...
        s->addSaveFunc([backgroundMusic, s] {
            if (backgroundMusic->getState() !=
                ...
                MusicManager::getInstance().update(0);
                s->setNeedsSaving();
            }
        });
    ...
    if (!Settings::getInstance()->getBool("ShowQuitMenu")) {
        ...
        row.makeAcceptInputHandler([window, this] {
            window->pushGui(new GuiMsgBox(
                ...
                [this] {
                    close(true);
                    if (Utils::Platform::quitES() != 0) {
                        LOG(LogWarning) << "Couldn't restart ES-DE";
                    }
                },
                _("NO"), nullptr));
        });
        ...
        if (Utils::Platform::hasSuspendSupport()) {
            row.elements.clear();
            row.makeAcceptInputHandler([window, this] {
                window->pushGui(new GuiMsgBox(
