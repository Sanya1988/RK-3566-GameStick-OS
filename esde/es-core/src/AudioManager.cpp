...
void AudioManager::init()
{
    ...
    if (Settings::getInstance()->getInt("SoundVolumeBackgroundMusic") > 100)
        Settings::getInstance()->setInt("SoundVolumeBackgroundMusic", 100);
    if (Settings::getInstance()->getInt("SoundVolumeBackgroundMusic") < 0)
        Settings::getInstance()->setInt("SoundVolumeBackgroundMusic", 0);

    setupAudioStream(sRequestedAudioFormat.freq);
    setupMusicStream(sRequestedAudioFormat.freq);
}
...
void AudioManager::deinit()
{
    ...
    SDL_FreeAudioStream(sMusicConversionStream);
    ...
    sMusicConversionStream = nullptr;
    sAudioDevice = 0;
}
...
void AudioManager::mixAudio(void* /*unused*/, Uint8* stream, int len)
{
    ...
    auto mixStreamData = [stream, len](SDL_AudioStream* conversionStream, const int volume,
                                       const bool muted) {
        if (conversionStream == nullptr)
            return false;

        int streamLength {SDL_AudioStreamAvailable(conversionStream)};
        if (streamLength <= 0)
            return false;

        const int chunkLength {std::min(streamLength, len)};
        std::vector<Uint8> converted(static_cast<size_t>(chunkLength));
        const int processedLength {SDL_AudioStreamGet(
            conversionStream, static_cast<void*>(&converted.at(0)), chunkLength)};

        if (processedLength < 0) {
            LOG(LogError) << "AudioManager::mixAudio(): Couldn't convert sound chunk:";
            LOG(LogError) << SDL_GetError();
            return SDL_AudioStreamAvailable(conversionStream) > 0;
        }

        SDL_MixAudioFormat(stream, &converted.at(0), sAudioFormat.format, processedLength,
                           muted ? 0 : volume);
        return processedLength > 0 || SDL_AudioStreamAvailable(conversionStream) > 0;
    };

    const bool hasVideoStream {mixStreamData(
        sConversionStream,
        static_cast<int>(Settings::getInstance()->getInt("SoundVolumeVideos") * 1.28f),
        sMuteStream)};
    const bool hasMusicStream {mixStreamData(
        sMusicConversionStream,
        static_cast<int>(Settings::getInstance()->getInt("SoundVolumeBackgroundMusic") * 1.28f),
        false)};

    if (!stillPlaying && !hasVideoStream && !hasMusicStream)
...
void AudioManager::setupMusicStream(int sampleRate)
{
    ...
    sMusicConversionStream = SDL_NewAudioStream(AUDIO_F32, 2, sampleRate, sAudioFormat.format,
                                                sAudioFormat.channels, sAudioFormat.freq);
    if (sMusicConversionStream == nullptr) {
        LOG(LogError) << "Failed to create music conversion stream:";
        LOG(LogError) << SDL_GetError();
    }
...
{
    if (sAudioDevice == 0 || sConversionStream == nullptr)
        return;
...
{
    if (sAudioDevice == 0 || sMusicConversionStream == nullptr)
        return;

    SDL_LockAudioDevice(sAudioDevice);

    if (SDL_AudioStreamPut(sMusicConversionStream, samples, count * sizeof(Uint8)) == -1) {
        LOG(LogError) << "Failed to put samples in the music conversion stream:";
        LOG(LogError) << SDL_GetError();
        SDL_UnlockAudioDevice(sAudioDevice);
        return;
    }
...
void AudioManager::clearMusicStream()
{
    ...
    SDL_LockAudioDevice(sAudioDevice);
    SDL_AudioStreamClear(sMusicConversionStream);
    SDL_UnlockAudioDevice(sAudioDevice);
}

int AudioManager::getPendingMusicBytes()
{
    if (sAudioDevice == 0 || sMusicConversionStream == nullptr)
        return 0;

    SDL_LockAudioDevice(sAudioDevice);
    const int pendingBytes {SDL_AudioStreamAvailable(sMusicConversionStream)};
    SDL_UnlockAudioDevice(sAudioDevice);
    return pendingBytes;
}
