...
namespace Utils
{
    namespace Localization
    {
        ...
        const std::vector<std::pair<std::string, std::string>> sSupportedLocales {
            {{"en"}, {"US"}}, {{"en"}, {"GB"}}, {{"ru"}, {"RU"}}, {{"ar"}, {"SA"}},
            {{"bs"}, {"BA"}}, {{"ca"}, {"ES"}}, {{"de"}, {"DE"}}, {{"es"}, {"ES"}},
            {{"fr"}, {"FR"}}, {{"hr"}, {"HR"}}, {{"it"}, {"IT"}}, {{"ja"}, {"JP"}},
            {{"ko"}, {"KR"}}, {{"nl"}, {"NL"}}, {{"pl"}, {"PL"}}, {{"pt"}, {"BR"}},
            {{"pt"}, {"PT"}}, {{"ro"}, {"RO"}}, {{"sr"}, {"RS"}}, {{"sv"}, {"SE"}},
            {{"zh"}, {"CN"}}, {{"zh"}, {"TW"}}};
        // clang-format on
...
#else
            const std::string requestedLocale {locale + ".UTF-8"};
            setenv("LANGUAGE", locale.c_str(), 1);
            setenv("LANG", std::string {"en_US.UTF-8"}.c_str(), 1);
            unsetenv("LC_ALL");
            setenv("LC_MESSAGES", std::string {"en_US.UTF-8"}.c_str(), 1);
            // Start from a known-good libc locale and then try to switch LC_MESSAGES only.
            // This keeps gettext functional even if the target rootfs only ships en_US locale
            // data while ES-DE bundles additional .mo catalogs.
            setlocale(LC_ALL, std::string {"en_US.UTF-8"}.c_str());
            setlocale(LC_MESSAGES, std::string {"en_US.UTF-8"}.c_str());
            if (setlocale(LC_MESSAGES, requestedLocale.c_str()) != nullptr) {
                setenv("LC_MESSAGES", requestedLocale.c_str(), 1);
            }
            else {
                LOG(LogWarning) << "Locale runtime \"" << requestedLocale
                                << "\" is unavailable, forcing gettext via LANGUAGE="
                                << locale;
            }
#endif
