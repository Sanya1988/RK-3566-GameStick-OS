...
public:
    ...
    // Used for background music playback.
    void setupMusicStream(int sampleRate);
    void processMusicStream(const void* samples, unsigned count);
    void clearMusicStream();
    int getPendingMusicBytes();
...
private:
    ...
    static inline SDL_AudioStream* sMusicConversionStream {nullptr};
